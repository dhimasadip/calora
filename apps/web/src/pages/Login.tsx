...
export default function Login() {
  ...
  return <main className="auth-page"><form className="auth-card" onSubmit={submit}><Link className="brand" to="/">cal<span>ora</span></Link><p className="eyebrow">Welcome back</p><h1>Good to see you.</h1><p>Pick up your gentle rhythm right where you left it.</p><label className="field-wrap"><span className="field-label">Email</span><input className={`wellness-input ${error ? 'input-error' : ''}`} type="email" value={email} onChange={(event) => { setEmail(event.target.value); setError(null) }} placeholder="you@example.com" autoComplete="email" aria-invalid={Boolean(error)} required /></label><label className="field-wrap"><span className="field-label">Password</span><input className={`wellness-input ${error ? 'input-error' : ''}`} type="password" value={password} onChange={(event) => { setPassword(event.target.value); setError(null) }} placeholder="Enter your password" autoComplete="current-password" aria-invalid={Boolean(error)} aria-describedby={error ? 'login-error' : undefined} required />{error && <span id="login-error" className="field-error" role="alert">{error}</span>}</label><button className="primary-action" disabled={loading}>{loading ? 'Signing in…' : 'Sign in'}</button><p className="auth-switch">New here? <Link to="/register">Create a free account</Link></p></form></main>
}
