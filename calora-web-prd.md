...
## CalTrack — Calorie & Exercise Tracker (Web)

| | |
|---|---|
| **Status** | Draft v1.0 |
| **Platform** | Responsive web application (desktop and mobile browsers) |
| **Backend** | Node.js (separate service) |
| **Date** | July 2026 |

> **Note on naming:** "CalTrack" is a placeholder working title used throughout this document — swap in the real app name before development/production launch.
...
### 1.1 Product Summary
CalTrack is a responsive web application for tracking daily caloric intake (food) and caloric expenditure (exercise/workouts). Users set a personal goal during onboarding (lose, maintain, or gain weight) based on their body stats, then log food and exercise throughout the day either **manually** or via an **AI agent** that estimates calories from a short text description. The web app visualizes intake vs. outtake across daily, weekly, monthly, and custom date ranges.
...
### 1.4 Product Access Model
- The v1 product is free to use and has no advertising or payment flow.
- AI-estimation calls are the primary variable cost, so usage is controlled through server-side daily quotas, caching, and rate limiting.
- The architecture may support future paid plans, but monetization is outside the v1 scope.
...
| Error-free session rate | ≥ 99.5% |

### 1.6 Platform Scope (v1)
- Responsive web application supporting current major desktop and mobile browsers.
- Accessible through a browser; no App Store or Play Store installation is required.
- Internet is required for AI estimation and server synchronization; basic manual logging should degrade gracefully during temporary connectivity loss (see NFR-2).
- The application may be installable as a Progressive Web App (PWA), but native mobile apps are outside the v1 scope.
...
| FR-12 | Notifications | Optional browser notifications and in-app reminders for logging meals / weigh-ins (configurable) | Should |
...
|---|---|---|
| NFR-1 | Performance | Dashboard and history pages load in ≤ 2s on a typical modern device with a normal connection |
| NFR-2 | Offline resilience | Manual-entry drafts and unsynced changes can be cached in the browser and synchronized when connectivity returns; AI estimation requires connectivity |
| NFR-3 | Security | Passwords hashed (bcrypt/argon2); all traffic over HTTPS/TLS; JWT access + refresh token auth |
| NFR-4 | Privacy | Health/body data is sensitive — support account deletion with full data purge and publish a clear privacy policy |
...
| NFR-7 | Web compatibility | Support the latest two major versions of Chrome, Edge, Firefox, and Safari; provide responsive layouts for desktop, tablet, and mobile viewport sizes |
| NFR-8 | Accessibility | Target WCAG 2.1 AA for keyboard navigation, semantic structure, focus states, labels, and adequate color contrast |
...
Log workouts by type, duration, and intensity. Manual path: user enters calories burned directly (or the system estimates via MET tables + user weight). AI path: user describes the activity ("ran 5k in 30 minutes"); AI returns an estimated calorie burn using MET-based reasoning combined with the user's profile.
...
### 3.7 Notifications
Configurable reminders (e.g., "Log your lunch", "Log today's weight") through in-app reminders and optional browser notifications when permission is granted.
...
```mermaid
flowchart TD
    A[Open Web App] --> B{Existing Account?}
...
1. User opens the web web app → lands on **Daily Dashboard** (today's intake, outtake, net, remaining calories, macro breakdown).
...
```mermaid
graph TB
    subgraph Client["Web App - Next.js / React"]
        UI[Responsive Presentation Layer]
        State[Client State and Server Cache]
        BrowserDB[(Browser Storage - IndexedDB)]
        SW[Service Worker / PWA Layer]
        UI --> State
        State --> BrowserDB
        SW --> BrowserDB
    end
    ...
    subgraph External["External Services"]
        ...
        Notifications[Web Push Provider - Optional]
        Analytics[Web Analytics / Error Monitoring]
    end

    State -- HTTPS/REST --> API
    SW -- Optional Web Push --> Notifications
    FoodSvc --> PG
    ExSvc --> PG
    ReportSvc --> PG
    ReportSvc --> Redis
...
### 5.2 Web App Architecture (Next.js / React)
Layered, feature-first structure:
- **Presentation layer** — responsive pages and reusable components for onboarding, dashboard, food/exercise entry, reports, history, and settings.
- **Routing and rendering** — Next.js App Router with a clear split between server-rendered public/auth pages and authenticated client-interactive application pages.
- **State management** — TanStack Query for server state and caching; lightweight local state through React hooks or Zustand where shared client state is required.
- **Domain layer** — framework-light TypeScript models, validation schemas, and use-cases such as `CalculateDailySummary` and `SubmitFoodEntry`.
- **Data layer** — typed API client for the Fastify backend; IndexedDB for temporary offline drafts and pending manual-entry synchronization.
- **Responsive design** — desktop-first dashboard layouts that collapse cleanly into tablet and mobile browser layouts.
- **PWA strategy** — optional installability, service-worker caching for static assets, and limited offline resilience for manual-entry drafts. AI estimation always requires a live network connection.
- **Authentication storage** — prefer secure, HTTP-only, SameSite cookies for refresh/session credentials; avoid exposing long-lived tokens to browser JavaScript.
...
- **Async work (optional)**: a queue (BullMQ + Redis) can offload AI estimation from the request/response cycle if latency becomes an issue, with the web client polling or receiving a WebSocket/server-sent-event update.

### 5.4 AI Agent Integration Design
1. Web client sends a free-text description of the food or activity, plus relevant profile data (e.g., body weight, for exercise estimates), to `POST /food-entries/ai-estimate` or `POST /exercise-entries/ai-estimate`.
...
5. Per-user daily AI-call quotas are enforced server-side (to control cost and prevent abuse).
...
- **LLM provider** — text-capable model for food/exercise estimation from natural-language descriptions; integration remains server-side and the API key is never exposed to the browser.
- **Web push provider (optional)** — browser notifications for reminders after the user grants permission.
- **Analytics and error monitoring** — privacy-conscious web analytics plus frontend/backend error tracking.
...
**food_items** *(optional master/cache table to speed up manual entry with autocomplete)*
| Column | Type | Constraints |
|---|---|---|
| id | UUID | PK |
...
**exercise_types** *(reference table for MET-based manual calculation)*
| Column | Type | Constraints |
|---|---|---|
| id | UUID | PK |
...
| Client state | React hooks; Zustand only where shared local state is needed |
| Forms / validation | React Hook Form + Zod |
| Browser storage | IndexedDB via Dexie for offline drafts/pending sync |
| Charts | Recharts or Apache ECharts |
| Navigation | Next.js App Router |
| Authentication | Backend-issued secure HTTP-only cookies; CSRF protection where applicable |
| PWA support | Web app manifest + service worker (optional for v1) |
...
| Monitoring/errors | Sentry or equivalent for both frontend and backend |
| Analytics | Plausible, PostHog, Mixpanel, or equivalent web analytics |
| Secrets management | Environment variables via the hosting provider's secret manager; never expose server API keys to the browser |
...
- v1 targets modern desktop and mobile web browsers through a responsive web interface.
...
- No advertising, payment, or subscription system is included in v1.
...
- Optional paid plan or subscription, only if a future business model requires it.
- Wearable or health-platform integration for automatic exercise/step import.
- Native mobile applications, if browser and PWA usage do not meet future needs.
